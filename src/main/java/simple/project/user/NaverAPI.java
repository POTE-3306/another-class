...
public class NaverAPI {
    public APIUserDTO getProfile(String accessToken) throws Exception {
        String header = "Bearer " + accessToken; // Bearer 다음에 공백 추가
        String apiURL = "https://openapi.naver.com/v1/nid/me";
        Map<String, String> requestHeaders = new HashMap<>();
        requestHeaders.put("Authorization", header);
        String responseBody = get(apiURL,requestHeaders);
        APIUserDTO apiUser = null;
        try {
            ...
            apiUser = new APIUserDTO(
                    responseMap.get("name"),
                    responseMap.get("email"),
                    2024 - Integer.parseInt(responseMap.get("birthyear")),
                    responseMap.get("gender").charAt(0),
                    responseMap.get("id"));
        } catch (Exception e) {
